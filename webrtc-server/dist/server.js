...
class MediaSoupManager {
    ...
    async createConsumer(transportId, producerId, rtpCapabilities) {
        ...
        if (!transport || !producer)
            throw new Error('Transport или Producer не найден');
        ...
        try {
            switch (data.type) {
                ...
                case 'connectTransport':
                    ...
                    break;
                case 'produce': {
                    const producer = await mediasoupManager.createProducer(data.transportId, data.rtpParameters, data.kind);
                    ws.send(JSON.stringify({ type: 'produced', data: { id: producer.id } }));
                    break;
                }
                case 'consume': {
                    const consumer = await mediasoupManager.createConsumer(data.transportId, data.producerId, data.rtpCapabilities);
                    ws.send(JSON.stringify({
                        type: 'consumed',
                        data: {
                            id: consumer.id,
